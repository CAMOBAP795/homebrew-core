class Latexml < Formula
  ...
  resource "Alien::Build" do
    url "http://www.cpan.org/authors/id/P/PL/PLICEASE/Alien-Build-1.78.tar.gz"
  ...
  def install
    ...
    resources.each do |r|
      r.stage do
        ...
        system "make"
        system "make", "install"
        ENV["PERL_CANARY_STABILITY_NOPROMPT"] = ""
      end
    end
    ...
    (libexec+"bin").find.each do |path|
      ...
      (bin+program).write_env_script("#{libexec}/bin/#{program}", :PATH => "#{Formula['perl'].opt_libexec/"bin"}:#{ENV['PATH']}", :PERL5LIB => ENV["PERL5LIB"])
    end
  end
